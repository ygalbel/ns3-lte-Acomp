...
void exportResults()
{
  std::string pathOut3 = pathOut;
        ...
        {
             out<<"part,filename,Distance,NumberUes,SimulationTime,IsComp,IsFlat,IsAllot,FlowID,Tx Bytes,Rx Bytes,Tx Packets,Rx Packets," 
...
{
   pathOut = "/media/sf_Lte/03_13_2013/";
  int isLog = 0;
  CommandLine cmd;
...
std::cout<<outputpath+"/Dlmacstats.txt";
  ...
  int UesNumber = 4; 
  std::map<int, unsigned int> mapByStartGroup;
  ...
  if (isComp==1)
  {
    ...
    int z = part;
      for (int i = 0; i < numberByApplication[z]; i++)
      {
        ...
        mapByStartGroup.insert ( std::pair<int,unsigned int>(currentNode.GetN(),part) );
    ...
    for( int z = 0; z < UesNumber; z++)
    {
      for (int i = 0; i < numberByApplication[z]; i++)
      {
        ...
        if(GetIndexOfClosestEnb(ueDevs[z].Get(i)) == part)
        {
          ...
          current.Add(ueDevs[z].Get(i));
          currentNode.Add(ueDevs[z].Get(i)->GetNode());
           mapByStartGroup.insert ( std::pair<int,unsigned int>(currentNode.GetN(),z) );
          //mapByStartGroup[currentNode.GetN()] = z
        }
		  }
	  }
  }
  ...
  // Assign IP address to UEs, and install applications
    ...
    for (int u = 0; u < numberByApplication[part]; u++)
    {
  ...
  for( int v = 0 ; v < numberByApplication[z] ; v++)
    ...
    /*  Ptr<Ipv4> ipv4 = ueNode->GetObject<Ipv4>();
      ...
      Ipv4Address addressServer = iaddr.GetLocal ();*/
        ...
        Ptr<Ipv4> ipv42 = endNode->GetObject<Ipv4>();
        Ipv4InterfaceAddress iaddr2 = ipv42->GetAddress (1,0);
        Ipv4Address addressClient = iaddr2.GetLocal ();
        ...
        UdpClientHelper client (addressClient, RHPort++);
        ...
        int startTime = 0;
        int stopTime = sec;
        clientAppsHTTP1.Start (MilliSeconds (startTime+ v * 11));
