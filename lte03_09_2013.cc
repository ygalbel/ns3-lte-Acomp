...
int main (int argc, char *argv[])
{
   ...
   pathOut = "/media/sf_Lte/03_08_2013/";
  int isLog = 0;
  CommandLine cmd;
  ...
  if(isLog == 1)
   { 
    ...
    LogComponentEnable("LteEnbMac", LogLevel(LOG_LEVEL_ALL | LOG_PREFIX_TIME| LOG_FUNCTION ));
...
std::cout<<outputpath+"/Dlmacstats.txt";
  ...
  if (isComp==1)
  {
    ...
    for( int z = 0 ; z< UesNumber; z++)
    {
      ...
      {
	   lteHelper->Attach (ueDevs[z].Get(i), enbDevs.Get (z));
   }
    }
...
//float ratio = 10;
  ...
  {*/
    ...
    /*  Ptr<Ipv4> ipv4 = ueNode->GetObject<Ipv4>();
      ...
      Ptr<Ipv4> ipv42 = ueNode->GetObject<Ipv4>();
...
/*      UdpEchoClientHelper ClientHTTP1 (addressServer, ++RHPort);
      ...
      ClientHTTP1.SetAttribute ("PacketSize", UintegerValue ((int)bitrate[z] * ratio));
      ApplicationContainer clientAppsHTTP1 = ClientHTTP1.Install (endNode);*/

      // Start in right fourth of time
      int startTime = sec * w;
      int stopTime = sec * (w +1) - 0.1;
