...
void exportResults()
{
        ...
        {
             out<<"Distance,NumberUes,SimulationTime,IsComp,FlowID,Tx Bytes,Rx Bytes,Tx Packets,Rx Packets," 
...
// Create a single RemoteHost
 ...
 /* Ptr<EpcTft> tftVIDEODOWN = Create<EpcTft> ();
  ...
  enum EpsBearer::Qci q3 = EpsBearer::NGBR_VIDEO_TCP_PREMIUM;
...
serverAppsHTTP2.Start (Seconds (0.01));
serverAppsHTTP3.Start (Seconds (0.01));
serverAppsHTTP4.Start (Seconds (0.01));
...
/*  UdpEchoServerHelper ueServerVIDEO (3478);      
  ...
  std::cout<<"NodeB"<<"     "<<"Number of applications\n";
  for(uint i = 0 ; i <4 ; i++)
  {
    std::cout<<i<<"  "<<enbNodes.Get(i)->GetNApplications()<<"\n";
  }
