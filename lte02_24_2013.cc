...
int main (int argc, char *argv[])
{
   ...
   pathOut = "/media/sf_Lte/02_24_2013/";
  int isLog = 0;
  CommandLine cmd;
  ...
  if(isLog == 1)
   { 
    LogComponentEnable("LteHelper", LogLevel(LOG_LEVEL_ALL | LOG_PREFIX_TIME| LOG_FUNCTION ));
    LogComponentEnable("FdBetFfMacScheduler", LogLevel(LOG_LEVEL_ALL | LOG_PREFIX_TIME| LOG_FUNCTION ));
    //LogComponentEnable ("LteStatsCalculator", LOG_LEVEL_INFO);
    LogComponentEnable ("PacketSink" , LogLevel(LOG_LEVEL_ALL | LOG_PREFIX_TIME| LOG_FUNCTION ));
...
uint16_t RHPort = 19294; //Port used for Google Talk voice/video -
uint16_t UEPort = 20000; //used for start of port range
...
bitrate[3] = WebBitrate;
  ...
  for( int z = 0 ; z< UesNumber; z++)
  {
    ...
    {
  //  std::cout<<"\naaaaaaa: "<<ueDevs[z].Get (v)->GetObject<LteUeNetDevice>()->GetImsi();

      //RHPort++;
      UEPort++;
      Ptr<Node> ueNode =  ueNodes[z].Get (v);//->GetNode();
      ...
      std::cout<<addriClient<<"   "<<addressServer<<"\n";   
      InetSocketAddress RH = InetSocketAddress (addressServer, RHPort++); //Remote host
      //InetSocketAddress UE = InetSocketAddress (addriClient, UEPort); //UE
      /*InetSocketAddress RHs = InetSocketAddress (addressServer, RHPort+1000); //Remote host
      InetSocketAddress UEs = InetSocketAddress (addriClient, UEPort+1000); //UE*/
      //remote host -> UE
            OnOffHelper onoffRH ("ns3::UdpSocketFactory", Address (RH));
      ...
      ApplicationContainer apps = onoffRH.Install (ueNode); //sink.Install (endNode);
      apps.Start (MilliSeconds (  v + z));
      //apps.Stop (Seconds (MilliSeconds(sec * 100)));
      //onoffRH.Start(Seconds(v / 10));
      //onoffAppsRH.Add (onoffRH.Install (ueNode));    
    
      //onoffRH.SetAttribute ("PacketSize", UintegerValue (7));
      PacketSinkHelper sink2 ("ns3::UdpSocketFactory", Address(RH));
      onoffAppsUE.Add (sink2.Install (endNode));

          /*  PacketSinkHelper sink3 ("ns3::UdpSocketFactory", Address(RH));
    onoffAppsUE.Add (sink3.Install (ueNode));*/

      /*ApplicationContainer clientApps = ueClient.Install (ueNode);*


      //std::cout<<GetIp(endNode)<<"  "<<RHPort<<"\t\t";*/

      /*PacketSinkHelper sink ("ns3::UdpSocketFactory", Address(UE));
    ApplicationContainer apps = sink.Install (endNode);
      ...
      *                       *
      *************************/
     //UdpEchoClientHelper ueClient (addriClient, UEPort);

      //UE -> remote host
      /*OnOffHelper onoffUE ("ns3::UdpSocketFactory", Address (RH));
      onoffUE.SetAttribute ("OnTime",  StringValue("ns3::ConstantRandomVariable[Constant=1000]")); 
      onoffUE.SetAttribute ("OffTime",  StringValue("ns3::ConstantRandomVariable[Constant=0]"));
      onoffUE.SetAttribute ("DataRate", DataRateValue (DataRate(bitrate[z])));
      //onoffUE.SetAttribute ("PacketSize", UintegerValue (7));
      onoffAppsUE.Add (onoffUE.Install (ueNode));*/
      //std::cout<<GetIp(ueNode)<<"  "<<UEPort<<"\n";
        
/*      clientApps.Start (Seconds (0.01));*/
    }
    //std::cout<<"\n\n";
  }
      onoffAppsRH.Start (Seconds (0.01));
      onoffAppsUE.Start (Seconds (0.02));
